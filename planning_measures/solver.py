...
def solve_deterministic(
    ...
    """
    Run deterministic ASP solving (single answer set).
    ...
    """
    ctl = clingo.Control([f"-c horizon={horizon}", "--warn=no-atom-undefined"])

    ctl.load(str(PLANNING_LP))
    ctl.load(str(REACHABILITY_LP))
    ctl.load(str(MEASURES_LP))
